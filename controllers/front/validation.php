...
class paymentprestashopValidationModuleFrontController extends ModuleFrontController
{
	/**
	...
	public function postProcess()
	{
		...
		{
			if ($module['name'] == 'paymentprestashop')
			{
				$authorized = true;
				break;
			}
		...
		$this->module->validateOrder($cart->id,Configuration::get('PS_OS_PAYMENTPRESTASHOP_AWAITING'), $total, $this->module->displayName, NULL, $mailVars, (int)$currency->id, false, $customer->secure_key);
		$cart = $this->context->cart;
		...
		$redirecturl = $this->context->link->getModuleLink('paymentprestashop', 'redirecturl', array("orderid"=>$this->module->currentOrder), true);
		...
		Tools::addCSS(_PS_BASE_URL_._MODULE_DIR_ . $this->module->name. '/views/css/paymentprestashop.css');
