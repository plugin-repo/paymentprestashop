<?php
class paymentprestashopRedirecturlModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
...
//		}
        ...
        if($generatedCheckSum == $checksum)
		 ...
		 //updating connection mode  and merchant id and tracking id(paymentprestashop_id).
		...
		if($retval == "false")
		{
            ...
            $objOrder->setCurrentState(Configuration::get('PS_OS_PAYMENTPRESTASHOP_SECURITY_ERROR'));
		...
		else if($retval == "true" && $status == "Y")
        {
            ...
            $history = new OrderHistory();
            $history->id_order = (int)$objOrder->id;		
            $objOrder->setCurrentState(Configuration::get('PS_OS_PAYMENTPRESTASHOP_SUCCESS'));			
        ...
        else if($retval == "true" && $status == "N")
        {
            ...
            $history->id_order = (int)$objOrder->id;
            $objOrder->setCurrentState(Configuration::get('PS_OS_PAYMENTPRESTASHOP_FAILED'));    
            $this->context->smarty->assign(array(
                    'desc' => $desc,
                    'amount' => $amount,
					'trackingid'=>$trackingid,
        ...
        else if($retval == "true" && $status == "C")
        {
            ...
            $history = new OrderHistory($id_order);
            $history->id_order = (int)$objOrder->id;
            $objOrder->setCurrentState(Configuration::get('PS_OS_PAYMENTPRESTASHOP_CANCELLED'));    
            $this->context->smarty->assign(array(
                    'desc' => $desc,
                    'amount' => $amount,
					'trackingid'=>$trackingid,
		...
		else if($retval == "true" && $status == "P")
        {
		   ...
		   //update status in paymentprestashop_orderDetails table.
			
			$order_reff=$_REQUEST['desc'];
			$objOrder = new Order($id_order); //order with id=1
            $history = new OrderHistory();
            $history->id_order = (int)$objOrder->id;
            $objOrder->setCurrentState(Configuration::get('PS_OS_PAYMENTPRESTASHOP_PARTIALLYSUCCESS'));			
            $this->context->smarty->assign(array(
                    'desc' => $desc,
                    'amount' => $amount,
					'trackingid'=>$trackingid,
	...
	else if ($retval == "true" && $status == "C")
        {
            ...
            $objOrder->setCurrentState(Configuration::get('PS_OS_PAYMENTPRESTASHOP_UNKNOWN_STATUS'));		
