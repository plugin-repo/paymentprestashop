...
$paymentprestashop = Module::getInstanceByName('paymentprestashop');
switch($action)
{
    case "get_states":
    ...
    case "get_terminal":
    	...
    	$sql = "SELECT `terminal_id`,`payment_type` FROM `"._DB_PREFIX_.$paymentprestashop->name."` WHERE `currency`='".$currency_iso."' AND `card_type`='".$card_type."'";
    	$result = Db::getInstance()->getRow($sql);
    	echo json_encode($result);
    	exit;
    	break;
    case "collect_card_types":
    	$card_types = $paymentprestashop->getCardTypes();
