...
$flight_mode = 'Y';
$delay_days = 2;
$Awaiting_Paymentprestashop_payment = 208;
$Paymentprestashop_Payment_Success = 209;
$Paymentprestashop_Payment_Partial_Success = 210;
$Paymentprestashop_Payment_Failed = 211;

$user_id = $orders['user_id']; // 1011
$trackingid = ($orders['paymentprestashop_id'] == NULL) ? '' : $orders['paymentprestashop_id'];
...
if (($current_state == $Awaiting_Paymentprestashop_payment || $current_state == $Paymentprestashop_Payment_Partial_Success || $current_state == $Paymentprestashop_Payment_Success) &&
	($isProcessed == 0)) {
	...
	$ssl = _PS_BASE_URL_ . __PS_BASE_URI__ . "modules/paymentprestashop/ssl.cer"; // maybe it is not needed. TBC
	...
	if ($transactionStatus == 'failed') {
		if (($connection_mode == 'on' && $current_state == $Awaiting_Paymentprestashop_payment) || 
			($connection_mode == 'off' && ($current_state == $Awaiting_Paymentprestashop_payment || $current_state == $Paymentprestashop_Payment_Success) && (strtotime($date_add) < strtotime('-' . $delay_days . ' days')))){
...
function updateTransaction($transactionId, $transactionTrackingId, $transactionStatus, $transactionStatusDescription)
{
	if (isset($transactionTrackingId)) {
		$updateSQL = "update " . _DB_PREFIX_ . "paymentprestashop_orderdetails set status='$transactionStatus', tracking_id='$transactionTrackingId',  where order_details_id='$transactionId'";
	} else {
		$updateSQL = "update " . _DB_PREFIX_ . "paymentprestashop_orderdetails set status='$transactionStatus' where order_details_id='$transactionId'";
	}
}
