...
class paymentprestashop extends PaymentModule
{
	...
	public function __construct()
	{
		...
		$this->name = 'paymentprestashop';
		$this->order = 'orders';
		$this->cart = 'cart_product';
        $this->paymentprestashopConfig = 'paymentprestashop_config';
		$this->tab = 'payments_gateways';
		$this->version = '1.0.0';
		$this->author = 'paymentprestashop';
		...
		$this->displayName = $this->l('Payment Prestashop');
		$this->description = $this->l('Accept payments for your products via Payment Prestashop.');
	...
	public function createOrderStatuses()
	{
		...
		$order_states = array(array(
				...
				'paid' => 0,
				'deleted' => 0,
				'status_config' => 'PS_OS_PAYMENTPRESTASHOP_AWAITING',
				'name' =>'Awaiting Payment Prestashop payment'
				),
				array(
				'invoice' => 1,
				'send_email' => 0,
				...
				'paid' => 1,
				'deleted' => 0,
				'status_config' => 'PS_OS_PAYMENTPRESTASHOP_SUCCESS',
				'name' =>'Payment Prestashop Payment Success'
				),
				array(
				'invoice' => 1,
				'send_email' => 0,
				...
				'paid' => 1,
				'deleted' => 0,
				'status_config' => 'PS_OS_PAYMENTPRESTASHOP_PARTIALLYSUCCESS',
				'name' =>'Payment Prestashop Payment Partial Success'
				),
				array(
				'invoice' => 1,
				'send_email' => 0,
				...
				'paid' => 0,
				'deleted' => 0,
				'status_config' => 'PS_OS_PAYMENTPRESTASHOP_FAILED',
				'name' =>'Payment Prestashop Payment Failed'
				),
				array(
				'invoice' => 1,
				'send_email' => 0,
				...
				'paid' => 1,
				'deleted' => 0,
				'status_config' => 'PS_OS_PAYMENTPRESTASHOP_REVERSED',
				'name' =>'Payment Prestashop Payment Reversed'
				),
				array(
				'invoice' => 1,
				'send_email' => 0,
				...
				'paid' => 1,
				'deleted' => 0,
				'status_config' => 'PS_OS_PAYMENTPRESTASHOP_CHARGEBACK',
				'name' =>'Payment Prestashop Payment Chargeback'
				),
				array(
				'invoice' => 1,
				'send_email' => 0,
				...
				'paid' => 1,
				'deleted' => 0,
				'status_config' => 'PS_OS_PAYMENTPRESTASHOP_SETTLED',
				'name' =>'Payment Prestashop Payment Settled'
				),
				array(
				'invoice' => 1,
				'send_email' => 0,
				...
				'paid' => 1,
				'deleted' => 0,
				'status_config' => 'PS_OS_PAYMENTPRESTASHOP_CANCELLED',
				'name' =>'Payment Prestashop Payment Cancel'
				),
				array(
				'invoice' => 1,
				'send_email' => 0,
				...
				'paid' => 1,
				'deleted' => 0,
				'status_config' => 'PS_OS_PAYMENTPRESTASHOP_UNKNOWN_STATUS',
				'name' =>'Payment Prestashop Payment Fail with Unknown Reason'
				)
			);
	...
	{
		$where = "`name` in ('Awaiting Payment Prestashop payment','Payment Prestashop Payment Success','Payment Prestashop Payment Partial Success','Payment Prestashop Payment Failed','Payment Prestashop Payment Reversed','Payment Prestashop Payment Chargeback','Payment Prestashop Payment Settled','Payment Prestashop Payment Cancel','Payment Prestashop Payment Fail with Unknown Reason')";
	...
	private function _postPaymentTypeProcess()
	{
		if(Tools::isSubmit('btnPaymentTypeSubmit'))
		{
			...
			{
				$SQL = "TRUNCATE TABLE `"._DB_PREFIX_."paymentprestashop_payment_types`";
				Db::getInstance()->execute($SQL);
				...
				foreach($payment_type_id as $key=>$value)
				{
					...
					Db::getInstance()->insert('paymentprestashop_payment_types',$data);
				}
						
				$SQL = "SELECT DISTINCT payment_type_id FROM `"._DB_PREFIX_."paymentprestashop_card_types` WHERE payment_type_id NOT IN (
					SELECT payment_type_id FROM `"._DB_PREFIX_."paymentprestashop_payment_types`
				)";
				$resultData=Db::getInstance()->executeS($SQL);
				foreach($resultData as $key=>$value)
				{
						$deleteSQL = "delete FROM `"._DB_PREFIX_."paymentprestashop_card_types` WHERE payment_type_id=".$value['payment_type_id'];
						Db::getInstance()->executeS($deleteSQL);
				}
				
				$SQL = "SELECT payment_type_id FROM `"._DB_PREFIX_."paymentprestashop_payment_types` WHERE payment_type_id NOT IN (
							SELECT payment_type_id FROM `"._DB_PREFIX_."paymentprestashop_card_types`
						)";
				...
				for($j=0;$j<=$countRow-1;$j++)
				{
					...
					$SQL = "Select DISTINCT card_type_id,card_type_name,language,logo from `"._DB_PREFIX_."paymentprestashop_card_types`";
					...
					foreach($resultData2 as $key=>$value)
					{
							...
							Db::getInstance()->insert('paymentprestashop_card_types',$data);
					}
	...
	private function _postCardTypeProcess()
	{
		if(Tools::isSubmit('btnCardTypeSubmit'))
		{
			...
			if (!count($this->_postErrors))
			{
				$SQL = "delete  from `"._DB_PREFIX_."paymentprestashop_card_types`";
				$result = Db::getInstance()->executeS($SQL);
				
				$SQL = "Select *  from `"._DB_PREFIX_."paymentprestashop_payment_types`";
				$result = Db::getInstance()->executeS($SQL);
				foreach($result as $key=>$data)
				{
				
					$payment_type_id=$data['payment_type_id'];
					$SQL = "TRUNCATE TABLE  from `"._DB_PREFIX_."paymentprestashop_card_types`";
					Db::getInstance()->execute($SQL);
					foreach($card_type_id as $key=>$value)
					{
   ...
   /**
	...
	private function _postCronTypeProcess()
	{
		if(Tools::isSubmit('btnCronTypeSubmit'))
		{
			...
			$url = _PS_BASE_URL_ .__PS_BASE_URI__."modules/paymentprestashop/createcron.php";
		...
		{			
			$url = _PS_BASE_URL_ .__PS_BASE_URI__."modules/paymentprestashop/statusupdate.php";
	...
	private function _displayPaymentprestashop()
	{
		...
		$card_type = $this->getcardTypePayment();
		
		$paymemt_type = $this->getPaymentTypes();
		
		$card_type = array_merge($card_type_initial,$card_type);
		$logo_path = _PS_BASE_URL_ .__PS_BASE_URI__."modules/paymentprestashop/images/";
	...
	public function getContent()
	{
		...
		$this->_html .= $this->_displayPaymentprestashop();
		return $this->_html;
	}
	...
	public function hookPayment($params)
	{
		...
		{
			return $this->display(__FILE__, 'paymentprestashop_supplierNotFound.tpl');
		}		
	}
	...
	public function hookDisplayPaymentEU($params)
	{
		...
		return array(
				'cta_text' => $this->l('Pay by paymentprestashop'),
	...
	public function checkModuleDispalyMode($cart)
	{
		...
		{
				$SQL = "SELECT *  FROM "._DB_PREFIX_."paymentprestashop WHERE seller_id = '".$array_container[$j]."' AND currency = '".$currency_iso_code."'";
				$result = Db::getInstance()->executeS($SQL);
				foreach($result as $key=>$paymentprestashop)
				{
					$comb1[$j][]= $paymentprestashop['payment_type'].'0'.$paymentprestashop['card_type'];	
				}	
		}
	...
	public function renderForm()
	{
		$fields_form = array(
				'form' => array(
						'legend' => array(
								'title' => $this->l('Payment Prestashop Merchant account Details'),
								'icon' => 'icon-envelope'
						),
						'input' => array(
								array(
										...
										'name' => 'toid',
										'desc' => 'Merchant Unique Id. Provided by Paymentprestashop. It will be fixed for all transactions Accept only [0-9] ',
								...
								array(
										...
										'name' => 'totype',
										'empty_message' => $this->l('This field is required.'),
										'desc' => $this->l('Provided by Paymentprestashop. Do not change the value paymentprestashop. It will be fixed for all transactions Accept only [0-9][A-Z][a-z]'),
										'required' => true
								),
								...
								array(
										...
										'name' => 'partenerid',
										'empty_message' => $this->l('This field is required.'),
										'desc' => $this->l('Provided by Paymentprestashop. Do not change the value. It will be fixed for all transactions Accept only [0-9][A-Z][a-z]'),
										'required' => true
								),
								...
								array(
										...
										'name' => 'key',
										'empty_message' => $this->l('This field is required.'),
										'desc' => $this->l('Add secret key provided to you by Paymentprestashop.'),
										'required' => true
								),
								array(
										'type' => 'text',
	...
	{
		$SQL = "SELECT * FROM `"._DB_PREFIX_."paymentprestashop_payment_types` WHERE `language`=1";
		return Db::getInstance()->executeS($SQL);
	}

	public function getCardTypes()
	{
		$SQL = "SELECT  * FROM `"._DB_PREFIX_."paymentprestashop_card_types` WHERE `language`=1";
		return Db::getInstance()->executeS($SQL);
	}
	public function getcardTypePayment()
	{
		
		$SQL = "SELECT DISTINCT card_type_name,card_type_id,logo FROM `"._DB_PREFIX_."paymentprestashop_card_types`";
        $result = Db::getInstance()->executeS($SQL);
